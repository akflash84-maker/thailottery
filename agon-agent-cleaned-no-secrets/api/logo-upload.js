...
export default async function handler(req, res) {
  ...
  try {
    if (req.method === 'DELETE') {
      await supabase.storage.from('media').remove(['logos/site-logo']);
      await supabase.from('settings_v2').update({ value: '' }).eq('key', 'logo_url');
    ...
    if (req.method === 'POST') {
      ...
      // Save to settings
      const { data: existing } = await supabase.from('settings_v2').select('id').eq('key', 'logo_url').maybeSingle();
      if (existing) await supabase.from('settings_v2').update({ value: url }).eq('key', 'logo_url');
      else await supabase.from('settings_v2').insert({ key: 'logo_url', value: url });
